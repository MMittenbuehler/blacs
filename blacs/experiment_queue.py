...
class QueueManager(object):
    ...
    def manage(self):
        ...
        while self.manager_running:
            ...
            # TODO: create custom context for exec. script
            # TODO: run execute script

            
            code = ""

            with h5py.File(path,'r') as runfile:
                code = runfile.attrs['executor_script']
            ...
            try:
                ...
                labscript.init_executor(self.shot_callback, path)

                exe = compile(
                    code, script_module.__file__, 'exec', dont_inherit=True
                )
            ...
            # FOREIGN SCRIPT IN RUNMANAGER (example):
            # TODO: do similar setup...
            # try:
    ...
    def shot_callback(self, runfile_path, shot_name, shot_id):
        shot_filepath = self.prepate_shot(runfile_path, shot_name, shot_id)
        return self.execute_shot(shot_filepath), shot_filepath

    def prepate_shot(self, runfile_path, shot_name, shot_id):
        ...
        with h5py.File(runfile_path,'r+') as runfile:
            ...
            shot_folder = runfile.attrs['shot_folder']
            ...
            runfile['shots'].create_group(f'{shot_id:04d}')
    ...
    def execute_shot(self, shot_path):
        ...
        try:  
            ...
            abort = False
            restarted = False
            self.set_status("Transitioning to buffered...", shot_path)
            ...
            with h5py.File(shot_path, 'r') as hdf5_file:
                devices_in_use = {}
                start_order = {}
                stop_order = {}
                ...
                if not transition_list:
                    ...
                    for name in start_groups.pop(min(start_groups)):
                        try:
                            ...
                            success = self.transition_device_to_buffered(name,transition_list,shot_path,restart_function)
                            if not success:
            ...
            self.set_status("Running (program time: %.3fs)..."%(time.time() - start_time), shot_path)
            ...
            self.set_status("Saving data...", shot_path)
        # End try/except block here
            ...
            self.manager_paused = True
            # is this a repeat?
            with h5py.File(shot_path, 'r') as h5_file:
                repeat_number = h5_file.attrs.get('run repeat', 0)
            # clean the h5 file:
            temp_path = tempfilename()
            self.clean_h5_file(shot_path, temp_path, repeat_number=repeat_number)
            try:
                shutil.move(temp_path, shot_path)
            except Exception:
                msg = ('Couldn\'t delete failed run file %s, ' % shot_path + 
                        'another process may be using it. Using alternate ' 
                        'filename for second attempt.')
                logger.warning(msg, exc_info=True)
                shutil.move(temp_path, shot_path.replace('.h5','_retry.h5'))
                shot_path = shot_path.replace('.h5','_retry.h5')
